...
namespace {
   ...
   std::complex<T> horner(const std::complex<T>& z, const T (&coeffs)[N]) noexcept
   {
      ...
      const T r = std::real(z) + std::real(z);
      const T s = std::real(z) * std::real(z) + std::imag(z) * std::imag(z);
      T a = coeffs[N - 1], b = coeffs[N - 2];
      ...
      return std::complex<T>(std::real(z)*a + b, std::imag(z)*a);
   }
...
{
   const float PI = 3.14159265f;
   const float re = std::real(z);
   const float im = std::imag(z);
   ...
   if (im == 0) {
      ...
      // Re(z) > 1
      return { Li2(re), -PI*std::log(re) };
   }

   const float nz = re*re + im*im;
   ...
   const std::complex<float> u2(u*u);
